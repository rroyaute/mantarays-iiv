...
library(patchwork); library(tidybayes); library(latex2exp)
```
...
check_model(glmm.rank.bin) %>% kable()
```
...
check_model(glmm.rank.poiss) %>% kable()
```
...
pp_check(brms.rank, ndraws = 100) %>% kable()
```
